...
# %%
# your code here
print(f'Amount of nulls {data.isnull().sum()}')
print(data.info())
print(data.describe())
...
scaled_data = pd.concat(
    [data.drop(columns=continuous_columns), scaled_df], axis=1)
...
prepped_data['NObeyesdad'] = prepped_data['NObeyesdad'].astype(
    'category').cat.codes
prepped_data.head()

# %%
prepped_data['NObeyesdad'].value_counts()
...
# %%
# your code here
test_sizes = [0.1, 0.2, 0.3, 0.5, 0.7]

for test_size_iter in test_sizes:
    X_train_iter, X_test_iter, y_train_iter, y_test_iter = train_test_split(
        X, y, test_size=test_size_iter, stratify=y, random_state=42)

    model_ova_iter = LogisticRegression(
        multi_class='ovr', max_iter=1000).fit(X_train_iter, y_train_iter)
    y_pred_ova_iter = model_ova_iter.predict(X_test_iter)
    print("One-vs-All (OvA) Strategy")
    print(
        f"Accuracy: {np.round(100*accuracy_score(y_test_iter, y_pred_ova_iter), 2)}%")

    model_ovo_iter = OneVsOneClassifier(LogisticRegression(
        max_iter=1000)).fit(X_train_iter, y_train_iter)
    y_pred_ovo_iter = model_ovo_iter.predict(X_test_iter)
    print("One-vs-One (OvO) Strategy")
    print(
        f"Accuracy: {np.round(100*accuracy_score(y_test_iter, y_pred_ovo_iter), 2)}%\n")
...
# %%
# your code here
model_ova_coef = model_ova.coef_
coefficients_ova = pd.Series(
    np.mean(np.abs(model_ova_coef), axis=0), index=X.columns)
coefficients_ova.sort_values().plot(kind='barh')
...
for estimator in model_ovo.estimators_:
    estimator_series.append(estimator.coef_[0].tolist())

estimator_series = np.array(estimator_series)
coefficients_ovo = np.mean(np.abs(estimator_series), axis=0)
coefficients_ovo = pd.Series(coefficients_ovo, index=X.columns)
coefficients_ovo.sort_values().plot(kind='barh')
plt.title("Feature Coefficients")
plt.xlabel("Coefficient Value")
plt.show()
...
def obesity_risk_pipeline(data_path, test_size, target_col):
    # First loading data.
    data = pd.read_csv(data_path)

    X = data.loc[:, ~(data.columns == target_col)].copy()
    y = data[[target_col]]
    ...
    # Standarizing continual data.
    col_to_standarize = X_train.select_dtypes(include=['float64']).columns

    sc = StandardScaler()
    X_train_norm = sc.fit_transform(X_train[col_to_standarize])
    X_train_norm = pd.DataFrame(
        X_train_norm, columns=sc.get_feature_names_out(col_to_standarize))

    X_test_norm = sc.transform(X_test[col_to_standarize])
    X_test_norm = pd.DataFrame(
        X_test_norm, columns=sc.get_feature_names_out(col_to_standarize))

    # One-hot encoding categorical data.
    col_to_onehot = X_train.select_dtypes(include=['object']).columns

    oh = OneHotEncoder(sparse_output=False, drop='first')
    X_train_oh = oh.fit_transform(X_train[col_to_onehot])
    X_train_oh = pd.DataFrame(
        X_train_oh, columns=oh.get_feature_names_out(col_to_onehot))

    X_test_oh = oh.transform(X_test[col_to_onehot])
    X_test_oh = pd.DataFrame(
        X_test_oh, columns=oh.get_feature_names_out(col_to_onehot))

    X_train = pd.concat([X_train_norm, X_train_oh], axis=1)
    X_test = pd.concat([X_test_norm, X_test_oh], axis=1)

    y_train['NObeyesdad'] = y_train['NObeyesdad'].astype('category').cat.codes
    y_test['NObeyesdad'] = y_test['NObeyesdad'].astype('category').cat.codes
    ...
    print("One-vs-All (OvA) Strategy")
    print(
        f"Accuracy: {np.round(100*accuracy_score(y_test, y_pred_ova), 2)}%\n")
    ...
    # Evaluating metrics for OvO
    print("One-vs-One (OvO) Strategy")
    print(
        f"Accuracy: {np.round(100*accuracy_score(y_test, y_pred_ovo), 2)}%\n")


obesity_risk_pipeline(file_path, test_size=0.2, target_col='NObeyesdad')
